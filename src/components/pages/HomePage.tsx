...
export default function HomePage (): JSX.Element {
  ...
  useEffect(() => {
    // scale number to 0.49 - 0.95 with inversion
    const normalizeNumber = function (value: number, min: number, max: number): number {
      let scaled_number = (value - min) / (max - min);
      return 0.95 - scaled_number * (0.49);
    };
    ...
    for (let y = 0; y < canvas_height; y++) {
      for (let x = 0; x < canvas_width; x++) {
        const probability = normalizeNumber(x, 0, canvas_width);
