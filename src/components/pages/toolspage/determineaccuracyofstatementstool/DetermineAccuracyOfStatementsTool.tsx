...
export default function DetermineAccuracyOfStatementsTool ({root_class = 'default', styles}): JSX.Element {
  ...
  const handle_submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await axios.post(EnvironmentService.getUrlBase() +
        '/api/1/determine-accuracy-query', {
          statements: statements,
          models: models_parameter
        }, {
          headers: {
      ...
      const processed_elements = data.map(function (item, index) {
        return `${item.model ?? 'Unknown model'} - ${(item.response?.content?.summary ?? 'error').toLowerCase()}`;
      });
  ...
  return (
    <>
      ...
      <div className={`${styles['jg-padding-single-indent-collapse']} ${styles['jg-width-66-collapse']}`}>
        Evaluate accuracy of provided statement(s).
      </div>
      <form onSubmit={handle_submit}>
        <div className={`${styles['jg-margin-top-50']}`}></div>

        <div className={`${styles['jg-padding-8-collapse']} ${styles['jg-width-66-collapse']}`}>
          <h2 className={`${styles['jg-overflow-hidden']} ${styles['js-asterix-decorate']}`}>
            Statements
          </h2>
        </div>
        <textarea
        ...
        <div className={`${styles['jg-response-data']} ${styles['jg-with-separated-lines']} ${styles['jg-response-data-fixed-height']}`}>
          {results.map(function (item, index) {
            return (<div className={`${styles['jg-line']}`} key={index}>{item}</div>);
          })}
        </div>
      </form>
    </>
